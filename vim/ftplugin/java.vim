...
if !empty(findfile('pom.xml', '.;'))
  ...
  def RunTests(method_arg: string, bang: bool)
    ...
    if !empty(mod_path) && mod_path != '.'
      const result = systemlist($'mvn -f {pom_path} help:evaluate -Dexpression=project.artifactId -q -DforceStdout 2>/dev/null')
      const mod = empty(result) ? '' : trim(result[0])
      ...
      if filereadable(mod_conf_path)
        config_path = $'{config_path}:{mod_conf_path}'
      endif
    endif
...
def Breakpoint()
  ...
  if empty(class)
    class = matchstr(file_path, '/src/\zs.\+\ze\.java$')
  endif
